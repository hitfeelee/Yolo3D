...
from models.losses import calc_loss, YoloLoss
...
def test(data,
    ...
    if dataloader is None:  # not training
        ...
        dataloader = kitti.create_dataloader(path, imgsz, batch_size, int(max(model.stride)),
                                       config=None, augment=False, cache=False, pad=0.5, rect=True)[0]
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm.tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # Run NMS
            t = torch_utils.time_synchronized()
            output = postprocess.apply_nms(inf_out, nc, conf_thres=conf_thres, iou_thres=iou_thres, merge=merge)
            ...
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    if pi.shape[0]:
                        ...
                        ious, i = metrics_utils.box_iou(pred[pi, :4], tbox[ti]).max(1)  # best ious, indices
        ...
        # Plot images
        if batch_i < 1:
            f = os.path.join(logdir, 'test_batch%g_gt.jpg' % batch_i)  # filename
            visual_utils.plot_images(img, targets, paths, f, names)  # ground truth
            f = os.path.join(logdir, 'test_batch%g_pred.jpg' % batch_i)
            visual_utils.plot_images(img, utils.output_to_target(output, width, height), paths, f, names)  # predictions
...
if __name__ == '__main__':
    ...
    opt.img_size = utils.check_img_size(opt.img_size)
