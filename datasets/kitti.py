...
def create_dataloader(path, imgsz, batch_size, stride,
                      config=None, augment=False, cache=False, pad=0.0, rect=False, is_training=False):
    dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                  augment=augment,  # augment images
                                  config=config,  # augmentation configure
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, config=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, is_training=False):
        root_path = path  # os-agnostic
        split = 'train' if is_training else 'test'
        file = os.path.join(root_path, 'ImageSets', split + '.txt')
        image_path = os.path.join(root_path, 'training', 'image_2')
        label_path = os.path.join(root_path, 'training', 'label_2')
        with open(file, 'r') as f:
            f = f.read().splitlines()
            self.img_files = [os.path.join(image_path, i + '.png') for i in f]
            self.label_files = [os.path.join(label_path, i + '.txt') for i in f]
        ...
        sp = os.path.join(root_path, 'cache',  split + '.shapes')   # shapefile path
        try:
        ...
        np_labels_path = os.path.join(root_path, 'cache', split + '_labels.npy')  # saved labels in *.npy file
                    ...
                    with open(file, 'r') as f:
                        ...
                        for x in f.read().splitlines():
                            ...
                            if line[0] not in classes:
                                continue
                            cls = classes.index(line[0])
                            xmin = float(line[4]) / shape[0]
                            ymin = float(line[5]) / shape[1]
                            xmax = float(line[6]) / shape[0]
                            ymax = float(line[7]) / shape[1]
            ...
            if l.shape[0]:
                ...
                if extract_bounding_boxes:
                    ...
                    for j, x in enumerate(l):
                        ...
                        b[2:] = b[2:] * 1.3 + 30  # pad
                        b = data_utils.xywh2xyxy(b.reshape(-1, 4)).ravel().astype(np.int)
    ...
    def __getitem__(self, index):
        ...
        if nL:
            ...
            labels[:, 1:5] = data_utils.xyxy2xywh(labels[:, 1:5])
